...
public class Easing {
	...
	public class Circular {
		...
		public static float Out(float k) {
			return Mathf.Sqrt(1f - ((k - 1f) * (k - 1f)));
		}

		public static float InOut(float k) {
			if ((k *= 2f) < 1f) return (1 - Mathf.Sqrt(1f - k * k)) / 2f;
			return (Mathf.Sqrt(1f - (k - 2f) * (k - 2f)) + 1f) / 2f;
		}
		public static float OutIn(float k) {
			if ((k *= 2f) < 1f) return (Mathf.Sqrt(1f - (k - 1f) * (k - 1f))) / 2f;
			return (1 - Mathf.Sqrt(1f - (k - 1) * (k - 1)) / 2f);
		}
